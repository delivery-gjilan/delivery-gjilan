...
- 2026-04-16: `Order.driverTakeHomePreview` GraphQL field added. The field is computed server-side from `SettlementCalculationEngine.calculateOrderSettlements()` output and exposed on the Order type for DRIVER/ADMIN/SUPER_ADMIN roles. The logic lives in `api/src/models/Order/resolvers/driverTakeHomePreview.ts` (extracted helper, unit-tested). Formula: cash retained after paying the business (for `CASH_TO_DRIVER` orders) plus `DRIVER/PAYABLE` settlements minus `DRIVER/RECEIVABLE` settlements. Returns `null` for non-driver queries. All eight driver-facing earnings surfaces in `mobile-driver` now prefer this field with `?? order.deliveryPrice` fallback.
