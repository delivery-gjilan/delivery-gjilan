...
interface OrderCardProps {
    ...
    onDoubleTap: (order: Order) => void;
    onReject: (orderId: string) => void;
    onAccept: (orderId: string) => void;
    onMarkReady: (orderId: string) => void;
    onAddTimePress: (order: Order) => void;
...
export default function OrdersScreen() {
    ...
    const { data, loading, refetch } = useQuery(GET_BUSINESS_ORDERS, {
        pollInterval: wsConnected ? 0 : 15000,
    });
    ...
    // Refetch when tab comes into focus (covers tapping a push notification to open the app)
    useFocusEffect(
        useCallback(() => {
            refetch();
        }, [refetch]),
    ...
    // Refetch when app returns to foreground (covers backgrounded → foreground transition)
    useEffect(() => {
    ...
    const allOrders = (data?.orders?.orders as unknown as Order[]) || [];
    if (__DEV__ && allOrders.length > 0) {
        console.log('[OrdersScreen] user.businessId:', user?.businessId);
        console.log('[OrdersScreen] order[0].businesses:', JSON.stringify(allOrders[0]?.businesses?.map((b: any) => b.business.id)));
    }
    ...
    const renderOrderCard = ({ item: order }: { item: Order }) => (
        <OrderCard
            ...
            onAddTimePress={(o) => {
                setAddTimeModalOrder(o);
                setAddTimeAmount(10);
                setCustomAddTime('');
            }}
            onToggleExpand={toggleExpandedItems}
        />
    );
    return (
        <SafeAreaView className="flex-1 bg-background" edges={['top']}>
            ...
            {isStoreClosed && (
                ...
                >
                    ...
                    <View style={{ backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 8, paddingVertical: 4, paddingHorizontal: 10 }}>
                        <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>
                            {t('orders.tap_to_open', 'Tap to Open')}
                        </Text>
                    </View>
                </TouchableOpacity>
            )}
            ...
            {hasPendingOrders && (
                ...
                >
                    ...
                    <Text style={{ color: '#000', fontWeight: '800', fontSize: 14, marginLeft: 10, flex: 1 }}>
                        {sortedOrders.filter((o) => o.status === 'PENDING').length}{' '}
                        {t('orders.new_orders_waiting', 'new order(s) waiting — double-tap to accept')}
                    </Text>
                </View>
            )}
