...
class ExpoMapboxNavigationViewController: UIViewController {
    ...
    func onRoutesCalculated(navigationRoutes: NavigationRoutes){
        ...
        navigationViewController.navigationView.speedLimitView.isHidden = true
        navigationViewController.navigationView.findViews(subclassOf: ResumeButton.self).forEach { $0.isHidden = true }
        ...
        navigationMapView!.mapView.mapboxMap.loadStyle(style!, completion: { _ in
            ...
            // Hide Mapbox logo and attribution button
            var ornamentOptions = navigationMapView!.mapView.ornaments.options
            ornamentOptions.logo.visibility = .hidden
            ornamentOptions.attributionButton.visibility = .hidden
            navigationMapView!.mapView.ornaments.options = ornamentOptions
        })
...
extension ExpoMapboxNavigationViewController: NavigationViewControllerDelegate {
    // Remove waypoint name text label (the blue pill showing the destination name)
    func navigationViewController(
        _ navigationViewController: NavigationViewController,
        waypointSymbolLayerWithIdentifier identifier: String,
        sourceIdentifier: String
    ) -> SymbolLayer? {
        var layer = SymbolLayer(id: identifier, source: sourceIdentifier)
        layer.textOpacity = .constant(0)
        return layer
    }
