...
import type { GetMySettlementsQuery, GetSettlementBreakdownQuery } from '@/gql/graphql';
...
export default function EarningsScreen() {
    ...
    const { data: breakdownData, loading: breakdownLoading, error: breakdownError, refetch: refetchBreakdown } = useQuery(
        GET_SETTLEMENT_BREAKDOWN,
                    ...
                    {isLoading ? (
                        ...
                        <View style={[es.flowCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                            <View style={es.flowCardHeader}>
                                <View style={[es.flowIconWrap, { backgroundColor: '#f59e0b20' }]}>
                                    <Ionicons name="warning-outline" size={18} color="#f59e0b" />
                                </View>
                                <View style={{ flex: 1 }}>
                                    <Text style={[es.flowStepLabel, { color: '#f59e0b' }]}>
                                            Failed to load earnings summary
                                    </Text>
                                    <Text style={[es.flowStepHint, { color: theme.colors.subtext }]}>
                                            Delivered orders are still shown below. Pull to refresh to retry.
                                    </Text>
                                </View>
                            </View>
                        </View>
