...
import type { GetMySettlementsQuery, GetSettlementBreakdownQuery, GetOrdersQuery } from '@/gql/graphql';
...
export default function EarningsScreen() {
    ...
    // Current-cycle breakdown: ignore period filter, always show debt since last paid settlement
    const lastPaidDate = cash?.lastPaidDate ? new Date(cash.lastPaidDate) : null;
    const currentCycleStart = lastPaidDate ? new Date(lastPaidDate.getTime() + 1).toISOString() : undefined;
    const { data: currentCycleData } = useQuery(
        GET_SETTLEMENT_BREAKDOWN,
        {
            variables: { isSettled: false, startDate: currentCycleStart },
            fetchPolicy: 'network-only',
            skip: cash === undefined, // wait until cashData has loaded
        },
    );
    const currentCycleBreakdown = currentCycleData?.settlementBreakdown ?? [];
    const currentCycleOwed = currentCycleBreakdown
        .filter((item) => item.direction === 'RECEIVABLE')
        .reduce((sum, item) => sum + Number(item.totalAmount ?? 0), 0);
    const currentCyclePlatformOwes = currentCycleBreakdown
        .filter((item) => item.direction === 'PAYABLE')
        .reduce((sum, item) => sum + Number(item.totalAmount ?? 0), 0);
    const currentCycleNet = currentCycleOwed - currentCyclePlatformOwes; // positive = driver owes platform
    ...
    const settlementOrders = useMemo(() => {
        ...
        unsettledSettlements.forEach((settlement) => {
    ...
    const categoryOrders = useMemo(() => {
        ...
        categorySettlements.filter((settlement) => isUnsettledStatus(settlement.status)).forEach((settlement) => {
            const orderId = String(settlement.order?.id ?? settlement.id);
            const existing = byOrder.get(orderId);
            if (existing) {
                existing.settlements.push(settlement);
                existing.totalReceivable += settlement.direction === 'RECEIVABLE' ? Number(settlement.amount ?? 0) : 0;
                existing.totalPayable += settlement.direction === 'PAYABLE' ? Number(settlement.amount ?? 0) : 0;
                if (new Date(settlement.createdAt).getTime() > new Date(existing.latestCreatedAt).getTime()) existing.latestCreatedAt = settlement.createdAt;
                return;
            }
            byOrder.set(orderId, {
                orderId,
    ...
    const getCategoryLabel = (cat: string): string => {
        const map: Record<string, string> = {
            ...
            CATALOG_REVENUE: t.earnings.cat_catalog ?? 'Catalog',
        };
        return map[cat] ?? cat;
    };
    ...
    return (
        <SafeAreaView style={[es.safe, { backgroundColor: theme.colors.background }]}>
            ...
            >
                ...
                {/* ── Current-cycle balance pill ── */}
                {cash && currentCycleNet !== 0 && (
                    <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                        <View style={[
                            es.cycleBalancePill,
                            {
                                backgroundColor: currentCycleNet > 0 ? "#ef444418" : "#22c55e18",
                                borderColor: currentCycleNet > 0 ? "#ef444440" : "#22c55e40",
                            },
                        ]}>
                            <Ionicons
                                name={currentCycleNet > 0 ? "warning-outline" : "checkmark-circle-outline"}
                                size={18}
                                color={currentCycleNet > 0 ? "#ef4444" : "#22c55e"}
                            />
                            <View style={{ flex: 1, marginLeft: 10 }}>
                                <Text style={[es.cycleBalanceLabel, { color: theme.colors.subtext }]}>
                                    {currentCycleNet > 0
                                        ? (t.earnings.current_cycle_owed ?? "Owed to platform this cycle")
                                        : (t.earnings.current_cycle_payable ?? "Platform owes you this cycle")}
                                </Text>
                                <Text style={[es.cycleBalanceHint, { color: theme.colors.subtext }]}>
                                    {lastPaidDate
                                        ? (t.earnings.since_last_settlement ?? "Since last settlement") + " · " + format(lastPaidDate, "MMM d, yyyy")
                                        : (t.earnings.all_time ?? "All time")}
                                </Text>
                            </View>
                            <Text style={[es.cycleBalanceAmount, { color: currentCycleNet > 0 ? "#ef4444" : "#22c55e" }]}>
                                {currentCycleNet > 0 ? "-" : "+"}{formatCurrency(Math.abs(currentCycleNet))}
                            </Text>
                        </View>
                    </View>
                )}
            ...
            >
                <Pressable style={es.modalBackdrop} onPress={() => { setSelectedSettlementOrder(null); setHighlightCategory(null); }} />
                <View style={[es.detailModalSheet, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                    {selectedSettlementOrder && (() => {
                        ...
                        const previewNetAmount = Number(fin?.driverNetEarnings ?? 0);
                        const displayNetAmount = hasSettlementLines ? netAmount : previewNetAmount;
                        const amountColor = displayNetAmount >= 0 ? theme.colors.income : '#f59e0b';
                        ...
                        return (
                            <>
                                ...
                                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingBottom: 4 }}>
                                    ...
                                    <Pressable onPress={() => { setSelectedSettlementOrder(null); setHighlightCategory(null); }}>
                                        <Ionicons name="close-circle" size={28} color={theme.colors.subtext} />
                                    </Pressable>
                                </View>
                                    ...
                                    {financialsLoading ? (
                                        ...
                                        <>
                                            ...
                                            <View style={[es.detailSection, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
                                                <View style={es.detailRow}>
                                                    ...
                                                    <Text style={[es.historyAmount, { color: amountColor }]}>
                                                        {displayNetAmount >= 0 ? '+' : '-'}{formatCurrency(Math.abs(displayNetAmount))}
                                                    </Text>
                                                </View>
                                                <View style={es.detailRow}>
                                                    ...
                                                    <View style={[es.methodBadge, {
                                                        backgroundColor: !hasSettlementLines
                                                            ? '#3b82f618'
                                                            : settlementsForOrder.some((line) => line.status !== 'PAID')
                                                                ? '#f59e0b18'
                                                                : theme.colors.income + '18',
                                                    }]}>
                                                        <Text style={[es.methodBadgeText, {
                                                            color: !hasSettlementLines
                                                                ? '#3b82f6'
                                                                : settlementsForOrder.some((line) => line.status !== 'PAID')
                                                                    ? '#f59e0b'
                                                                    : theme.colors.income,
                                            ...
                                            <View style={[es.detailSection, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
                                                ...
                                                {hasSettlementLines ? settlementsForOrder.map((line) => {
                                                    ...
                                                    <>
                                                        ...
                                                        <View style={es.detailRow}>
                                                            <Text style={[es.detailLabel, { color: theme.colors.subtext }]}>
                                                                {t.earnings.cat_markup ?? 'Markup'} / {t.earnings.commission ?? 'Commission'}
                                                            </Text>
                                                            <Text style={[es.detailValue, { color: '#f59e0b' }]}>
                                                                -{formatCurrency(Number(fin?.amountToRemitToPlatform ?? 0))}
                                                            </Text>
                                                        </View>
                                                        {Number(fin?.driverTip ?? 0) > 0 && (
                                                            <View style={es.detailRow}>
                                                                <Text style={[es.detailLabel, { color: theme.colors.subtext }]}>
                                                                    {t.earnings.tip_label ?? 'Tip'}
                                                                </Text>
                                                                <Text style={[es.detailValue, { color: theme.colors.income }]}>
                                                                    +{formatCurrency(Number(fin?.driverTip ?? 0))}
                                                                </Text>
                                                            </View>
                                                        )}
                                                    </>
...
const es = StyleSheet.create({
    ...
    cycleBalanceLabel: { fontSize: 13, fontWeight: "600" },
    cycleBalanceHint: { fontSize: 11, marginTop: 2 },
    cycleBalanceAmount: { fontSize: 18, fontWeight: "800" },
