...
} from '@/database/schema/promotionsV2';
...
export class PromotionEngineV2 {
    ...
    async getApplicablePromotions(
        ...
        const allPromos = await this.db
            ...
            .where(
                and(
                    eq(promotionsV2.isActive, true),
                    or(
                        isNull(promotionsV2.startsAt),
                        lte(promotionsV2.startsAt, now)
                    ),
                    or(
                        isNull(promotionsV2.endsAt),
                        gte(promotionsV2.endsAt, now)
                    ),
                    or(
                        isNull(promotionsV2.minOrderAmount),
                        lte(promotionsV2.minOrderAmount, String(cart.subtotal))
                    )
                )
            );
    ...
    async recordUsage(
        ...
        for (const promoId of promotionIds) {
            ...
            await this.db
                .update(promotionsV2)
                .set({
                    currentGlobalUsage: sql`${promotionsV2.currentGlobalUsage} + 1`,
                    totalUsageCount: sql`${promotionsV2.totalUsageCount} + 1`,
                    totalRevenue: sql`${promotionsV2.totalRevenue} + ${orderSubtotal}`,
                })
                .where(eq(promotionsV2.id, promoId));
