...
export const createPromotionV2: NonNullable<MutationResolvers['createPromotionV2']> = async (_parent, { input }, { userData }) => {
  ...
  const [promo] = await db.insert(promotionsV2).values({
    name: input.name,
