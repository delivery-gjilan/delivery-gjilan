...
export const orders = pgTable(
    'orders',
    {
        ...
        // Price breakdown:
        // basePrice              = sum of items' effective price × qty + options' extraPrice × qty
        //                          (uses salePrice if on sale, i.e. respects discounts)
        // markupPrice            = sum of products' markup amounts × qty (platform margin, additional amount only, ≥ 0)
        // actualPrice            = final item price after promotions (what customer pays for items)
        // originalDeliveryPrice  = calculated delivery fee from distance/zone business logic (before promotions)
        // deliveryPrice          = final delivery fee (after promotions, may be 0 for free delivery)
        ...
        originalDeliveryPrice: numeric('original_delivery_price', {
            mode: 'number',
            precision: 10,
            scale: 2,
        }).notNull(),
