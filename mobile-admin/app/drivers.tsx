...
export default function DriversScreen() {
    ...
    const { data, loading, refetch }: any = useQuery(GET_DRIVERS);
    const drivers = data?.drivers || [];

    return (
        <SafeAreaView className="flex-1" style={{ backgroundColor: theme.colors.background }}>
            <Stack.Screen options={{ title: 'Drivers', headerShown: true, headerStyle: { backgroundColor: theme.colors.background }, headerTintColor: theme.colors.text }} />
            <FlatList
                data={drivers}
