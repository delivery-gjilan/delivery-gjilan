...
import { GET_BUSINESSES } from '@/graphql/businesses';
import { getInitials } from '@/utils/helpers';

export default function BusinessesScreen() {
    const theme = useTheme();
    const router = useRouter();
    const { data, loading, refetch }: any = useQuery(GET_BUSINESSES);
    const businesses = data?.businesses || [];

    return (
        <SafeAreaView className="flex-1" style={{ backgroundColor: theme.colors.background }}>
            <Stack.Screen options={{ title: 'Businesses', headerShown: true, headerStyle: { backgroundColor: theme.colors.background }, headerTintColor: theme.colors.text }} />
            <FlatList
                data={businesses}
