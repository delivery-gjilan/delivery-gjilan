...
import { ADMIN_GET_STORE_STATUS, ADMIN_UPDATE_STORE_STATUS } from '@/graphql/operations/admin/misc';
...
export default function AdminMoreScreen() {
    ...
    const { data: storeData }: any = useQuery(ADMIN_GET_STORE_STATUS);
    ...
    const handleToggleStore = async () => {
        try {
            await updateStoreStatus({ variables: { input: { isStoreClosed: isStoreOpen } } });
        } catch {
    ...
    const handleLanguageToggle = () => {
        setLanguageChoice(languageChoice === 'en' ? 'al' : 'en');
    };
    ...
    return (
        <SafeAreaView className="flex-1" style={{ backgroundColor: theme.colors.background }}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
                {/* Admin badge header */}
                <View className="mx-4 mt-3 mb-5 rounded-2xl p-4 flex-row items-center" style={{ backgroundColor: theme.colors.card }}>
                    <View
                        ...
                        <Text className="text-xl font-bold" style={{ color: theme.colors.primary }}>
                            {adminGetInitials(user?.firstName, user?.lastName)}
                        </Text>
                    </View>
                    <View className="flex-1">
                ...
                <MenuSection
                    title="MANAGEMENT"
                    items={[
                        { icon: 'business', label: 'Businesses', sub: 'View & manage restaurants', color: '#8b5cf6', onPress: () => router.push('/admin/businesses' as any) },
                        { icon: 'bicycle', label: 'Drivers', sub: 'View driver activity', color: '#3b82f6', onPress: () => router.push('/admin/drivers' as any) },
                        { icon: 'people', label: 'Users', sub: 'Customer management', color: '#22c55e', onPress: () => router.push('/admin/users' as any) },
                    ]}
                />
                ...
                <MenuSection
                    title="TOOLS"
                    items={[
                        { icon: 'notifications', label: 'Notifications', sub: 'Push notification campaigns', color: '#f59e0b', onPress: () => router.push('/admin/notifications' as any) },
                        { icon: 'wallet', label: 'Settlements', sub: 'Payment settlements', color: '#14b8a6', onPress: () => router.push('/admin/settlements' as any) },
                    ]}
                />
                    ...
                    items={[
                        {
                            ...
                            trailing: (
                                <View className="px-2.5 py-1 rounded-lg" style={{ backgroundColor: `${theme.colors.primary}15` }}>
                                    <Text className="text-xs font-semibold" style={{ color: theme.colors.primary }}>{themeLabel}</Text>
                                </View>
                            ),
                        },
                        {
                            ...
                            trailing: (
                                <View className="px-2.5 py-1 rounded-lg" style={{ backgroundColor: `${theme.colors.primary}15` }}>
                                    <Text className="text-xs font-semibold" style={{ color: theme.colors.primary }}>{langLabel}</Text>
                                </View>
                            ),
                        },
                    ]}
